...
class ExampleCardBlockContainer extends Component {
    ...
    render() {
        ...
        if (filteredCards.length <= 0 || tagFilteredCards.length <= 0){
            displayContent = <p class="guide-text">Can't find example? Visit our Community Forum below to request for a new example!</p>
        }
        else{
        ...
        return (
            <Fragment>
                <input class="wick-filter" type="text" placeholder="Filter examples" value={this.state.filterText} onChange={this.updateFilter}></input>
                <br></br>
